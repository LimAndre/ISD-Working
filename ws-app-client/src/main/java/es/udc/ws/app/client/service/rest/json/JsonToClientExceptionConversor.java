...
public class JsonToClientExceptionConversor {
    ...
    public static Exception fromForbiddenErrorCode(InputStream ex) throws ParsingException {
        try {
            ...
            if (rootNode.getNodeType() != JsonNodeType.OBJECT) {
                ...
                String errorType = rootNode.get("errorType").textValue();
                if (errorType.equals("CursoNotRemovable")) {
    ...
    private static ClientCourseFullException toCourseFullException(JsonNode rootNode) {
        ...
        return new ClientCourseFullException(cursoId);
    }
    ...
    public static Exception fromGoneErrorCode(InputStream ex) throws ParsingException {
        try {
            ...
            if (rootNode.getNodeType() != JsonNodeType.OBJECT) {
                ...
                String errorType = rootNode.get("errorType").textValue();
                if (errorType.equals("CourseClosed")) {
                    return toCourseClosedException(rootNode);
                } else {
                    throw new ParsingException("Unrecognized error type: " + errorType);
                }
            }
    ...
    private static ClientCourseClosedException toCourseClosedException(JsonNode rootNode) {
        Long inscripcionId = rootNode.get("inscripcionId").longValue();
        return new ClientCourseClosedException(inscripcionId);
    }
